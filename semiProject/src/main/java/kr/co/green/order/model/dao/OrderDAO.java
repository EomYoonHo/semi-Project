...
public class OrderDAO {
	...
	public int orderEnroll(Connection con, OrderDTO orderDTO) {
		String query = "INSERT INTO orders " + "		 VALUES(orders_seq.nextval,"
				+ "					 ?,?,?,?,?,?,?,?,?,?,?," + "						SYSDATE)";
	...
	public OrderDTO selectOrder(Connection con, String o_imp_uid) {
		String query = "SELECT o_idx," + "					o_imp_uid," + "					o_people,"
				+ "					o_paid_amount," + "					o_point," + "					p_idx,"
				+ "					o_name," + "					m_no," + "					o_buyer_phone,"
				+ "					o_buyer_name," + "					o_buyer_email," + "					o_period,"
				+ "					o_paid_at" + "					FROM orders" + "		WHERE o_imp_uid= ?";
